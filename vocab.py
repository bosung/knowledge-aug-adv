...
class Vocab:
    def __init__(self, txt_file):
        ...
        # add sep token
        self.sep_tok = '<sep>'
        self.add_word(self.sep_tok)
        self.sep_idx = self.w2vvocab[self.sep_tok]
    ...
    def save_reduced_vec(self, target):
        with open("en_%s_word_vec_reduced.txt" % target, "w", encoding='utf-8') as f:
            f.write("%s %s\n" % (str(len(self.w2vvocab)), "300"))
            for k in self.w2vvocab:
                _idx = self.w2vvocab[k]
                vector = self.embeddings[_idx]
                line = [k] + [str(x) for x in vector]
                # print(" ".join(line))
                f.write(" ".join(line) + "\n")
        print("Saved %d word vector reduced version!" % len(self.w2vvocab))
